...
* 비공개 메소드의 인터페이스는 테스트 하지 않지만 `구현은 테스트 대상`이다
...
3. 모든 test case가 성공하는 것을 확인하며 `리펙토링`

#### OOP는 인터페이스를 노출하고 구현을 숨긴다 -> 은닉화
* test case는 인터페이스(public 멤버)에 의존적이며, 구현에 독립적
...
* 따라서 비공개 메소드에 담긴 코드는 이미 `test case를 통해 완전히 검증된 후 재배치 된 것`
  * test case가 비공개 메소드의 인터페이스에 대해 전혀 알지 못함에도 불구하고
* test case가 공개 인터페이스에만 의존하기 때문에 내부 구현은 숨겨지고 언제든 더 나은 구조로 변경될 수 있다


<br>

## 정리
* 비공개 메소드는 인터페이스에 기반하여 비공개 메소드에 독립적으로 테스트 하자
* 비공개 메소드는 리펙토링 과정에서 생기는 내부 구현이기 때문
  * 기능이 돌아갈 정도만 구현해서 쭈욱 작성하고, 리펙토링하면서 설계를 가다듬어 가자
