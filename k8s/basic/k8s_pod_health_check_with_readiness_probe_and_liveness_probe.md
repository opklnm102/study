...
* application은 실행되다가 broken state가 되어 restart를 제외하고 복구할 수 없는 경우 사용하면 유용
  * deadlock 등
  * kubernetes에서 자동으로 감지하여 restart
...
```
* readiness probe
  * Pod Start 후 60초부터 10초마다 수행
  * timeout 3초로 1번 성공시 health, 5번 연속 실패시 unhealth
* liveness probe
  * Pod Start 후 65초부터 20초마다 수행
  * timeout 1초로 1번 성공시 health, 3번 연속 실패시 unhealth
