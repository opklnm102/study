...
* API 설계자가 예외를 던지는 메소드를 선언할 때는 이유가 있으므로 `무시하지 말자`
```java
// 예외 무시
try {
    ...
...
* 예외의 목적은 `예외적인 상황을 처리`하라는 것
* 최소한 catch 블록에는 예외를 무시한 이유를 설명하는 `주석이라도 한 줄` 들어 있어야 한다

### 예외를 무시해도 좋은 상황
* FileInputStream을 닫을 때
* 입력 스트림이므로 파일의 상태를 변경하지 않는다
* 어떤 복구 처리도 필요 없다
* 필요한 정보를 읽었으므로 진행중인 작업을 중지할 이유도 없다
* 이런 경우라도 나중에 조사 가능하도록 예외를 처리하는 것이 좋다

### 정리
* 디버깅에 도움이 될만한 정보를 보존하면서, 외부로 예외를 전파시키기만 해도 충분하다
